...
class FetchData {
  ...
  fetch(request, config, options, done) {
    async.auto({
      ...
      method: cb => {
        this.getMethod(request, config, cb);
      },
      inject: cb => {
  ...
  getMethod(request, config, done) {
    const resolveArgument = (arg) => {
      // resolve if its a url path parameter or query:
      if (typeof arg === 'string' && arg[0] === '{') {
        const variable = _.trim(arg, ['{', '}']);
        arg = _.get(request, variable);
      }
      return arg;
    };
    ...
    this.map(obj, (method, name, cb) => {
      const reached = Hoek.reach(this.server.methods, method);
      if (!reached) {
        this.server.log(['error', 'hapi-views'], { error: 'Method not found', method });
        return cb(new Error('Method not found'));
      }
      if (config.args) {
        const newArgs = _.map(config.args, resolveArgument);
        newArgs.push(cb);
        reached.apply(this.server, newArgs);
      } else {
        reached(cb);
      }
    }, done);
