...
class FetchData {
  ...
  fetch(request, config, options, done) {
    async.auto({
      ...
      data: ['inject', 'yaml', 'api', 'method', (results, cb) => {
        ...
        const context = Object.assign({}, results, {
          request: {
  ...
  getApi(apis, request, allDone) {
    ...
    this.map(apis, (api, varName, done) => {
  ...
  getMethod(methods, request, done) {
    this.map(methods, (method, name, cb) => {
      ...
      if (method.args) {
        ...
        const args = varson({ args: Hoek.clone(method.args) }, context, this.varsonSettings).args;
        args.push(cb);
