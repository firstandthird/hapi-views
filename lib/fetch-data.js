...
class FetchData {
  ...
  fetch(request, config, options, done) {
    async.auto({
      ...
      data: ['inject', 'yaml', 'api', 'method', (results, cb) => {
        ...
        const out = varson(data, context, this.varsonSettings);
        cb(null, out);
      }],
  ...
  getApi(apis, request, allDone) {
    ...
    const executeApiCall = (url, options, callback) => {
    ...
    // if apis is just a string without a property name, it defaults to top-level (for now):
    if (typeof apis === 'string') {
      return executeApiCall(varson({ url: apis }, request, this.varsonSettings).url, { json: true }, (err, result) => {
        if (err) {
          return allDone(err);
        }
    ...
    // if it's a top-level api call:
    if (apis.url) {
      const options = { json: true };
      if (apis.headers) {
        options.headers = apis.headers;
      }
      return executeApiCall(varson({ url: apis.url }, request, this.varsonSettings).url, options, (err, result) => {
        if (err) {
          return allDone(err);
        }
        return allDone(null, result);
    ...
    // otherwise it's an object mapping property names to api url
    async.mapValues(apis, (api, varName, done) => {
      const options = { json: true };
      if (api.headers) {
        options.headers = api.headers;
      }
      const url = (typeof api === 'string') ? varson({ url: api }, request, this.varsonSettings).url : varson({ url: api.url }, request, this.varsonSettings).url;
      executeApiCall(url, options, done);
    }, (err, all) => {
      allDone(err, all);
    });
  }
  ...
  getMethod(methods, request, done) {
    ...
    this.map(obj, (method, name, cb) => {
      ...
      if (method.args) {
        const args = varson({ args: Hoek.clone(method.args) }, { request: Hoek.clone(request) }, this.varsonSettings).args;
        args.push(cb);
